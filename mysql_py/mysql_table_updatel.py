...
# mysql_py = '''insert into zhengjie (user_id,user_name,password,user_nick,card_num) values
# ('111','asddsa','sada','fsaas','12324324')'''
# try:
#     for n in range(100):
...
#
#del删除数据
# dl = "delete from zhengjie where user_id = '120'"   #赋值变量于命令
# result.execute(dl)         #执行命令
# db.commit()
# data = result.fetchall()      #执行的数据拿出来
# print(data)

#update
up = '''update zhengjie set user_id ='130'where user_id ='111' '''
result.execute(up)
db.commit()
data = result.fetchall()      #执行的数据拿出来
print(data)
#select 选择
# select = '''select*from zhengjie'''
